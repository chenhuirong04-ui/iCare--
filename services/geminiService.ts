...
`;
 ...
 const clean1688Keyword = (text: string) => {
  if (!text) return "";

  return text
    .replace(/[，,。.!！？?；;：:“”"'‘’（）()\[\]{}<>]/g, " ")
    .replace(/\b(供应商|厂家|工厂|价格|成本|控制|人民币|以下|以内|优先|出口|经验|寻找|请寻找|有出口经验|为优先)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
};

const build1688Url = (keyword: string, maxPrice = 6) => {
...
const normalized1688 = (hunterResult.marketplaces1688 || [])
  .map((item: any, idx: number) => {
    ...
    return {
      ...
      url: build1688Url(baseKeyword, 6),
      source: build1688Url(baseKeyword, 6),
      sourceType: "1688",
