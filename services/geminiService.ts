...
async function analyzeSearchIntent(q: string): Promise<SearchIntentAnalysis> {
  const ai = getAI();
  const fallback = fallbackAnalyzeSearchIntent(q);

  const prompt = `
你是全球采购搜索意图分析器。目标市场：迪拜/UAE进口商，需要在以下国家寻找可出口工厂：
${REGION_CONTEXT}
...
`;
  ...
  try {
    ...
    return {
      originalQuery: q,
      factoryKeyword: parsed.factoryKeyword?.trim() || fallback.factoryKeyword,
      keyword1688: cleanKeyword(parsed.keyword1688 || "") || fallback.keyword1688,
...
export const createGCIQuote = async (
  ...
  const newGciQuote: GCIQuote = {
    id: `GCI-Q-${new Date().toISOString().slice(2, 10).replace(/-/g, "")}-${Math.random()
      .toString(36)
      .slice(2, 6)
      .toUpperCase()}`,
...
- 不允许编造官网；只有真实来源中明确出现的官网才填写 website，否则 website 留空
...
`;
  ...
  try {
    hunterResult = JSON.parse(
      resp.text || '{"factories":[],"marketplaces1688":[],"suggestedKeywords":[]}'
    );
  } catch {
    hunterResult = {
      factories: [],
  ...
  const normalizedFactories = (hunterResult.factories || []).map((f: any, idx: number) => ({
    ...
    whatsapp: f.whatsapp || "",
    email: f.email || "",
   website: /^https?:\/\/.+\..+/i.test(String(f.website || "")) ? f.website : "",
   isOfficialWebsite: /^https?:\/\/.+\..+/i.test(String(f.website || "")) && f.isOfficialWebsite === true,
...
export const analyzeRFQImages = async (i: any): Promise<RFQProduct[]> => {
  ...
  parts.unshift({
    text: "提取 RFQ 产品事实清单。包含中文名、规格、数量、单位、材质、包装形式、每箱数量、外箱尺寸、毛重、净重、备注。返回 JSON 数组。",
  } as any);
